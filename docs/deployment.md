...
Целевой сервер — `185.79.138.118`. На нём уже работает Traefik в Docker network `web_gateway`, поэтому Smart Replenishment разворачивается как отдельный Compose-проект и не открывает новые внешние порты. Дашборд доступен по адресу:
