...
## 7. Деплой на отдельный VPS

Для VPS с ограниченной RAM обучение выполняется локально; целевой сервер получает только Docker-образ и два готовых Parquet-артефакта. Не разворачивайте проект на сервере, где работает другой production-сервис, без отдельного подтверждения его ресурсов, сети и домена. Чек-лист подготовки описан в [`docs/deployment.md`](docs/deployment.md).
