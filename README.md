...
Для VPS с ограниченной RAM обучение выполняется локально; сервер получает только Docker-образ и два готовых Parquet-артефакта. На настроенном VPS проект доступен через HTTPS на `nip.io` без покупки отдельного домена. Полный сценарий описан в [`docs/deployment.md`](docs/deployment.md).
